...
void sedna_execute_module(SEDNAMODULE *mod)
{
    ...
    executeByteCode(&stack, mod, mainMeth.bytecode, mainMeth.bytecode_sz);
}
...
void executeByteCode(Stack *stack, SEDNAMODULE *mod, char *bytecode, int size)
{
    ...
    for (x = 0; x < size; x++)
    {
        ...
        else if (bytecode[x] == 0x51) //load local variable
        {
            int tA, tB;
            if (debug == 1)
            {
        ...
        else if (bytecode[x] == 0x52) //load local variable
        {
            int tA, tB;
            if (debug == 1)
            {
        ...
        else if (bytecode[x] == 0x53) //load local variable
        {
            int tA, tB;
            if (debug == 1)
            {
...
void Sedna_Init()
{
    ...
    sedna_install_function("Kernel", "display::setx", sedna_system_call);
    sedna_install_function("Kernel", "display::sety", sedna_system_call);
    sedna_install_function("Kernel", "display::setcolorfg", sedna_system_call);
    sedna_install_function("Kernel", "display::setcolorbg", sedna_system_call);
    sedna_install_function("Kernel", "display::clear", sedna_system_call);
...
void sedna_call(char *path, SEDNAMODULE *mod, Stack *args)
{

    char *module = strremovefirst(strtok_once(path, "]"));

    char *func = strremovefirstCount(path, strlen(module) + 2);
    //kprintf("Module:%s, Func:%s\n", module, func);
    int i, j, x;
    if (strcmp(module, "this") == 0)
    {
        for (x = 0; x < mod->method_cnt; x++)
        {
            if (strcmp(mod->method[x].name, func) == 0)
            {
                //kprintf("\"%s\" ", mod->method[x].name);
                executeByteCode(args, mod, mod->method[x].bytecode, mod->method[x].bytecode_sz);
                break;
            }
        }
    }
